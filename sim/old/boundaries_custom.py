...
class RectangularBoundary(Boundary):
    ...
    def _get_closest(self, pos: np.ndarray) -> np.ndarray:
        pos = np.copy(pos)
        coords = np.concatenate([self.bottom_left, self.top_right])
        deltas = coords - np.concatenate([pos, pos])
        min_idx = np.argmin(np.abs(deltas))
        pos[min_idx % 2] = coords[min_idx]
        return pos
...
if __name__ == "__main__":
    ...
    Boundarys = [
        CircularBoundary([0, 0], 10.0),
        RectangularBoundary([-10, -10], [+10, +10]),
        PolygonalBoundary([[-10, 0], [-10, -5], [10, -5], [5, 10]]),
    ]
    ...
    for i, obs in enumerate(Boundarys):
