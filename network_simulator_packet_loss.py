...
def setup_network():
    ...
    net_sim.set_node_positions(positions={id: pos for id, pos in enumerate(positions)})
    time.sleep(1.0)
...
net_sim.update_sim_time()
print(f"Real elapsed time: {time.time() - t0:.2f} s")
print(f"NS-3 elapsed time: {net_sim.ns3_time:.2f} s")
