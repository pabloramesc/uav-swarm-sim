...
num_drones = 16
num_users = num_drones * 4
num_obstacles = 2
size = 1e3
num_episodes = 1000
max_episode_time = 5 * 60

config = SDQNConfig(target_velocity=10.0, target_height=0.0)


obstacles_layer = ObstaclesLayerFactory(label="Obstacles Map")
...
frame_factory = FrameGeneratorFactory(
    ...
    layer_factories=[obstacles_layer, neighbors_layer, users_layer],
)
...
for episode in range(1, num_episodes + 1):
    sim.create_random_environment(num_obstacles=0, boundary_size=(1e3, 2e3))
    sim.initialize()
    gui.initialize()
    ...
    while True:
        ...
        if sim.sdqn_brain.dones is not None and np.any(sim.sdqn_brain.dones):
            break
