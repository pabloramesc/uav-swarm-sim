...
def main():
    ...
    try:
        while True:
            ...
            if keep_sending and sim_time >= next_tx_time:
                packet_count += 1
                msg = f"{packet_count}${sim_time}"
                packet = SimPacket(
                    node_id=user0_id,
                    src_addr=user0_addr,
                    dst_addr=user1_addr,
                ...
                print(f"[TX] Packet {packet_count} sent at {sim_time:.2f} s")
                next_tx_time = sim_time + np.random.normal(TX_INTERVAL, TX_INTERVAL / 10)
            ...
            for packet in packets:
                ...
                rtt = sim_time - tx_time
                print(f"[RX] Packet {packet_id} received, RTT: {rtt:.3f} s")
                received_ids.add(packet_id)

            # Exit condition: stop when all packets are received
            if not keep_sending and len(packets) == 0:
                break
